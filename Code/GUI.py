...
def toggle():
    if toggle_btn.config('relief')[-1] == 'raised':
        toggle_btn.config(
            relief="sunken", background='#e94235', text='Classifier')
    else:
...
def GradesSheet_Window():
    ...
    l1 = tk.Label(my_w, text='Upload your Sheet 😀',
                  font=my_font1, background='white')
    l1.grid(row=1, column=10)
    b1 = tk.Button(my_w, text='Upload image', background='#e2d3e4',
                   width=20, command=lambda: upload_file())
    b1.grid(row=2, column=10)
    toggle_btn = tk.Button(my_w, text="OCR", width=12, relief="raised",
                           fg='white', background='#4285f4', command=lambda: toggle())
    toggle_btn.grid(row=4, column=10)
    my_w['bg'] = '#2a5289'
    l1_Bubble = tk.Label(my_w, text='Wait Results....',
                         font=my_font2, background='white')
...
def Bubble_Window():
    try:
        ...
        my_font2 = ('times', 12, 'bold')
        l1_Bubble = tk.Label(
            my_w_Bubble, text='Upload your Sheet 😀', font=my_font1, background='white')
        ...
        l2_Bubble = tk.Label(
            my_w_Bubble, text='Wait Results....', font=my_font2, background='white')
...
def main():
    ...
    img = ImageTk.PhotoImage(Image.open("Code/panel.jpg"))
