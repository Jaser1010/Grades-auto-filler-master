...
def GradesSheet(path, OCR_flag):
    ...
    cut_contours(img_final_bin=img_final, orignal_img=img_output, loaded_svc=loaded_svc,
                 loaded_knn=loaded_knn, loaded_rf=loaded_rf, loaded_lr=loaded_lr, OCR_flag=OCR_flag)


def run_all_bubble_sheets(path):
    # path='./testCases/'
...
def Run_one_bubble_sheet(path):
    ...
    paper = extract_the_paper_from_image(image, None)
    answers, id = get_student_answer(
        paper, loaded_svc, loaded_knn, loaded_rf, loaded_lr)
